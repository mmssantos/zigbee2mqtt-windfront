import { deleteTerms, getTerms } from "./poeditor.mjs";
...
async function main(projectId, apiToken) {
    ...
    const terms = await getTerms(projectId, apiToken, "en");
    const cleanableContexts = ['"featureDescriptions"', '"featureNames"', '"settingsSchemaDescriptions"', '"settingsSchemaTitles"'];
    ...
    const deletionResults = await deleteTerms(projectId, apiToken, termsToClean);
